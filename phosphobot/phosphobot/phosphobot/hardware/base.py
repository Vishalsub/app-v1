...
class BaseManipulator(BaseRobot):
    ...
    def __init__(
        ...
        **kwargs: Optional[Dict[str, str]],
        ...
        # Only load URDF in simulation if we're in simulation mode or only_simulation is True
        if only_simulation or cfg.ONLY_SIMULATION:
            if reset_simulation_bool:
                self.sim.reset()

            logger.info(f"Loading URDF file for simulation: {self.URDF_FILE_PATH}")
            ...
            # Hardware mode - set defaults, these will be updated when connecting to real hardware
            logger.info("Hardware mode - skipping simulation URDF loading")
            self.p_robot_id = None
            self.num_joints = 0
